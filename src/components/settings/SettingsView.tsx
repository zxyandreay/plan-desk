import { Download, FileJson, Info, Monitor, Moon, Sun, Trash2, Upload } from 'lucide-react'
...
export function SettingsView() {
  ...
  return (
    <div className="pd-page pd-page-narrow space-y-6">
      <header>
        <p className="pd-eyebrow">Local-first settings</p>
        <h2 className="pd-page-title">Settings</h2>
      ...
      <section className="pd-panel p-5">
        <h3 className="pd-section-title">Preferences</h3>
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <div className="space-y-1.5 text-sm">
            <span className="font-medium text-[color:var(--pd-foreground)]">Theme</span>
            <div className="pd-segmented flex w-full">
              {[
                ...
                return (
                  <button
                    key={option.value}
                    type="button"
                    className={`pd-segment flex flex-1 items-center justify-center gap-2 ${
                      active ? 'pd-segment-active' : ''
                    }`}
                    aria-pressed={active}
                    onClick={() =>
                      updateSettings({ theme: option.value as typeof data.settings.theme })
                    }
                  >
      ...
      <section className="pd-panel p-5">
        <h3 className="pd-section-title">Backup / Import</h3>
      ...
      <section className="pd-panel p-5">
        <h3 className="pd-section-title">Data tools</h3>
        <div className="mt-4 flex flex-wrap gap-2">
      ...
      <section className="rounded-xl border border-[color:var(--pd-border)] bg-[color:var(--pd-accent)] p-5">
        <div className="flex gap-3">
          <Info className="mt-0.5 h-5 w-5 shrink-0 text-[color:var(--pd-accent-foreground)]" />
          <div>
            <h3 className="text-base font-semibold text-[color:var(--pd-foreground-strong)]">
              How local file links work
            </h3>
            <p className="mt-2 text-sm leading-6 text-[color:var(--pd-muted-foreground)]">
      ...
      <section className="pd-panel p-5">
        <h3 className="pd-section-title">About</h3>
